...
class GoogleTokenController extends Controller
{
    ...
    // updated after use harddelete for users
     public function verifyGoogleToken(Request $request)
    {
        $idToken = $request->input('id_token');

        if (!$idToken) {
            return $this->error(Lang::get('validation.custom.token_missing', [], 'Token is missing'), null, self::$responseCode::HTTP_BAD_REQUEST);
        }
        ...
        try {
            ...
            if ($response->failed()) {
                return $this->error(Lang::get('validation.custom.invalid_token', [], 'Invalid token'), null, self::$responseCode::HTTP_UNAUTHORIZED);
            }

            $payload = $response->json();

            if ($payload['iss'] !== 'https://accounts.google.com') {
                return $this->error(Lang::get('validation.custom.invalid_issuer', [], 'Invalid token issuer'), null, self::$responseCode::HTTP_UNAUTHORIZED);
            }

            if ($payload['exp'] < time()) {
                return $this->error(Lang::get('validation.custom.token_expired', [], 'Token has expired'), null, self::$responseCode::HTTP_UNAUTHORIZED);
            }
            
            $user = User::where('email', 'like', '%' . mb_strtolower($payload['email']) . '%')->first();
            ...
            if ($user) {
                ...
                $user = User::create([
                    'name' => $payload['name'] ?? Lang::get('validation.custom.default_user_name', [], 'User'),    
                    'email' => $payload['email'],
                    'email_verified_at' => now(),
                    'google_id' => $payload['sub'],
                ]);
