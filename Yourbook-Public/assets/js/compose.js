  ...
  if (input.files && input.files[0]) {
    ...
    reader.onload = function (e) {
      ...
      $('.remove-file').on('click', function () {
        $('#feed-upload-input-1, #feed-upload-input-2')
          .val('')
          .attr('disabled', false)
...
if ($('#compose-card').length) {
  ...
  $('#album-date button').on('click', function () {
    ...
    $(this).closest('.album-date').find('.control');

    var newInput = $("<input class='input mt-3' type='text' placeholder='Name Of Internal Book'>");

    $('#album-date .internal_book_container').append(newInput);
